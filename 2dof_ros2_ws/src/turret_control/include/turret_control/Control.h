...
{
public:
    Observer(StateSpace& ss_ref, std::shared_ptr<jsonRead> config);
...
{
public:
LQR(StateSpace& ss_ref, std::shared_ptr<jsonRead> config);
private:
    std::unique_ptr<jsonRead> configReader =
        std::make_unique<jsonRead>("/home/alex/Desktop/2DOF-Turret/2dof_ros2_ws/src/turret_control/lqr_observer_config.json");
};
